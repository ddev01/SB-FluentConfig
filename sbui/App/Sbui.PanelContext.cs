...
{
    /// <summary>
    /// Panel context logic extracted from Sbui: WithPanel, WithVisibility, WithRepeatableRows,
    /// panel/saveKey stacks, GetTargetPanel, GetFullSaveKey.
    /// </summary>
    public partial class Sbui
    {
        private sealed class SbuiPanelContext
        {
            ...
            internal Panel GetTargetPanel(string tabName)
            {
                ...
                return _sbui._visibilityOverridePanel ?? (Panel)_sbui._tabManager?.GetPanel(tabName);
            }
            ...
            internal void WithVisibility(string toggleSaveKey, string tabName, Action content, bool inverted = false)
            {
                ...
                var toggle = _sbui._controlRegistry.Get<ToggleSwitch>(toggleSaveKey);
                if (toggle == null)
            ...
            private void DeleteRowData(string saveKey, int rowIndex)
            {
                ...
                if (rowIndex >= 0 && rowIndex < arr.Count)
                {
                    ...
                    if (_sbui._existingSettings == null) _sbui._existingSettings = new JObject();
                    _sbui._existingSettings[saveKey] = arr;
                    _sbui.MarkDirty();
                }
            }
            ...
            {
                if (_sbui._existingSettings == null) return new JArray();
                var token = _sbui._existingSettings[saveKey];
                if (token is JArray arr) return arr;
                if (token != null)
                {
                    try { return JArray.Parse(token.ToString()); }
                    catch (Exception ex) { Log("LoadRowsData parse error: " + ex.Message); }
                }
                return new JArray();
            }

            private void Log(string message) => Sbui.LogInternal(message);
