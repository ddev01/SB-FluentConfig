...
{
    public class DecimalStepperElement : UIElement
    {
        ...
        public DecimalStepperElement(string title, string description, string tabName, string saveKey, double min, double max, double step, double defaultValue, string visibilityKey = null)
        {
            Title = title;
        ...
        public override void Render(IRenderContext context)
        {
            ...
            var val = context.Settings?[SaveKey] != null && (context.Settings[SaveKey].Type == Newtonsoft.Json.Linq.JTokenType.Float || context.Settings[SaveKey].Type == Newtonsoft.Json.Linq.JTokenType.Integer)
                ? context.Settings[SaveKey].ToObject<double>() : DefaultValue;
            ...
            row.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
            var tb = SbuiComponentFactory.CreateDecimalStepperTextBox(SaveKey, val);
            tb.TextChanged += (s, e) =>
            {
                context.MarkDirty();
            ...
            var minusBtn = new Button
            {
                ...
                Width = 28, Height = 28, Margin = new System.Windows.Thickness(0, 4, 2, 0), Padding = new System.Windows.Thickness(0)
            };
            minusBtn.Click += (s, e) => { if (double.TryParse(tb.Text, out var v)) { v = Math.Max(Min, v - Step); tb.Text = v.ToString("G"); context.MarkDirty(); } };
            var plusBtn = new Button
            {
                Content = new System.Windows.Controls.TextBlock { Text = "+", FontSize = 16, HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center },
                Width = 28, Height = 28, Margin = new System.Windows.Thickness(0, 4, 0, 0), Padding = new System.Windows.Thickness(0)
            };
